...
angular.module('mean.ffxivCrafter').directive('projectItemGathers', function () {
  return {
    ...
    controller: ItemGathersController
  }
})

ItemGathersController.$inject = ['$scope', '$http', 'deliveryService', 'StockService']

function ItemGathersController ($scope, $http, deliveryService, StockService) {
  $scope.gatherFilter = ''
